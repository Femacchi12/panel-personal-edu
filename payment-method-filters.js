(() => {
  ...
  async function load(force=false){
    ...
    const data=await getData(force);return parseRows(data?.sources?.[`${FINANCE_ID}|Movimientos!A:Z`]||[]);
  }
  ...
  function ensureBar(){let bar=document.getElementById('paymentMethodFilterBar');if(bar)return bar;const filterBar=document.getElementById('filterBar');if(!filterBar)return null;bar=document.createElement('section');bar.id='paymentMethodFilterBar';bar.className='filter-bar section-filter-bar';bar.hidden=true;bar.innerHTML='<div class="section-filter-grid"></div>';filterBar.insertAdjacentElement('afterend',bar);return bar;}
  const defs=[{key:'account',label:'Cuenta / medio',getter:account},{key:'method',label:'Modalidad',getter:method}];
  const selected=(view,key)=>state[view]?.[key]||[];
  function clearPaymentState(view){if(!state[view])return;state[view].account=[];state[view].method=[];if(window.__PAYMENT_FILTER_STATE__?.view===view)window.__PAYMENT_FILTER_STATE__={view,account:[],method:[]};}

  function renderFilter(def,options,view){
    const values=selected(view,def.key),summary=!values.length?'Todos':values.length===1?values[0]:`${values.length} seleccionados`;
  ...
  document.addEventListener('click',event=>{
    ...
    if(event.target.closest('#refreshBtn'))schedule(250,true);
  },true);

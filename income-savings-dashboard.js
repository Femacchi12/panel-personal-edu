(() => {
  ...
  function render(data){
    ...
    const total=sum(focus,'total'),liquid=sum(focus,'liquid'),expenses=sum(focus,'expenses'),saving=liquid-expenses,invested=sum(focus,'invested'),post=saving-invested;
    const savingRate=liquid?saving/liquid:null;
    ...
    root.innerHTML=`<section class="income-savings-dashboard">
      ...
      <div class="income-flow-grid">
        ...
        ${card('Gastos',money(expenses,cur,r),'Movimientos reales registrados')}
        ${card('Ahorro antes de invertir',money(saving,cur,r),liquid?`Tasa ${pct(savingRate)}`:'Sin ingreso confirmado',saving>=0?'green':'bad')}
        ${card('Aportes netos a inversiones',money(invested,cur,r),'Cambio de capital base; no incluye valorización','gold')}
        ${card('Saldo después de invertir',money(post,cur,r),'Ahorro generado − aportes netos',post>=0?'blue':'bad')}
      </div>

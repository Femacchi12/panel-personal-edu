(() => {
  ...
  function markup(trip){
    ...
    return `<section class="trip-expense-module" aria-label="Gastos reales del viaje">
      <div class="trip-expense-head"><div><span>GASTOS REALES DEL VIAJE</span><strong>${esc(trip.meta.name)}</strong><small>${esc(period)} · fuente exclusiva: Finanzas Edu / Movimientos</small></div><span class="trip-source-badge">${trip.rows.length} movimientos</span></div>
      <div class="trip-kpis"><div><span>Gastado hasta ahora</span><strong>${esc(money(trip.total))}</strong></div><div><span>Días con gastos</span><strong>${trip.days}</strong></div><div><span>Promedio por día</span><strong>${esc(money(avg))}</strong></div><div><span>Categorías</span><strong>${trip.categories.size}</strong></div></div>
      <div class="trip-grid"><div class="panel"><div class="panel-header"><div class="panel-title"><strong>Por categoría</strong><span>Distribución del gasto real etiquetado</span></div></div><div class="trip-break-list">${breakdown(trip.categories)}</div></div><div class="panel"><div class="panel-header"><div class="panel-title"><strong>Por medio de pago</strong><span>Cómo se financió la estadía</span></div></div><div class="trip-break-list">${breakdown(trip.payments)}</div></div></div>
      <div class="panel trip-detail-panel"><div class="panel-header"><div class="panel-title"><strong>Detalle de gastos del viaje</strong><span>Últimos ${Math.min(14,trip.rows.length)} movimientos</span></div></div><div class="table-scroll expanded"><table class="trip-expense-table"><thead><tr><th>Fecha</th><th>Descripción</th><th>Categoría</th><th>Cuenta</th><th>Modalidad</th><th>Monto COP</th></tr></thead><tbody>${detailRows(trip.rows)}</tbody></table></div></div>
    </section>`;
  }

(() => {
  ...
  function account(row){
    ...
    if(n.includes('efectivo'))return'Efectivo'; if(n.includes('nequi'))return holder.includes('ro')?'Nequi Ro':'Nequi Edu';
    if(n.includes('arq'))return'ARQ Edu'; if(n.includes('nu')){if(n.includes(' ro')||holder.includes('rocio')||holder==='ro')return'Nu Ro';return'Nu Edu';}
    return raw||'Sin especificar';
  }
  function method(row){
    const explicit=String(row['Modalidad de pago']||'').trim(); if(explicit)return explicit;
    const raw=norm(row['Cuenta / Tarjeta']); if(raw.includes('credito'))return'Crédito'; if(raw.includes('transferencia'))return'Transferencia'; if(raw.includes('debito'))return'Débito'; if(raw.includes('efectivo'))return'Efectivo';
    const q=num(row.Cuotas); if(q>0&&(raw.includes('nu')||raw.includes('arq')))return'Crédito'; return'Sin especificar';
  }
  ...
  function categoryList(flowRows,movements,year){
    ...
    const known=ORIGINAL_ORDER.filter(x=>[...set].some(y=>norm(y)===norm(x))); const extra=[...set].filter(x=>!known.some(y=>norm(y)===norm(x))).sort((a,b)=>a.localeCompare(b,'es',{sensitivity:'base'})); return [...known,...extra];
  }
  ...
  function salaryMap(concepts){
    ...
    const byYear=new Map(); concepts.forEach(r=>{const k=monthKey(r.Mes); if(!k)return; const cop=num(r['Sueldo COP']),usd=num(r['Sueldo USD (equiv. COP)']); if(cop>0){const y=k.slice(0,4);if(!byYear.has(y))byYear.set(y,[]);byYear.get(y).push(cop);} if(cop+usd>0){map.set(k,cop+usd);source.set(k,'Nómina COP + Fibrazo LLC USD');}});
  ...
  function currentMonthValues(movements,current){
    const rows=movements.filter(r=>matchesActiveFilters(r)&&monthKey(r['Mes consumo'])===current);
    ...
    const financed=rows.filter(r=>norm(method(r))==='credito').reduce((s,r)=>s+num(r['Monto COP']),0);
    const total=rows.reduce((s,r)=>s+num(r['Monto COP']),0);
    const summary=new Map([
      ['fijo',fixed],['fijo + super',fixed+supermarket],['variable',variable],['variable - super',Math.max(0,variable-supermarket)],['egresos efectivos',Math.max(0,total-financed)],['egresos financiados',financed],['egresos totales',total]
    ]);
  ...
  function renderDetail(host,movements,cat,key){
    const rows=movements.filter(r=>isReal(r)&&norm(r['Categoría'])===norm(cat)&&monthKey(r['Mes consumo'])===key).sort((a,b)=>(effectiveDate(a)?.getTime()||0)-(effectiveDate(b)?.getTime()||0));
  ...
  function render(host,detail,data){
    const year=selectedYear(), months=monthList(year), current=currentMonthKey();
    const historical=historicalMap(data.flowRows), currentData=currentMonthValues(data.movements,current), salary=salaryMap(data.concepts), categories=categoryList(data.flowRows,data.movements,year);
    const values=new Map(historical);
    if(current.startsWith(year+'-')) categories.forEach(cat=>values.set(`${current}|${norm(cat)}`,currentData.byCat.get(norm(cat))||0));
    ...
    host.innerHTML=`<div class="panel-header"><div class="panel-title"><strong>Matriz mensual por categoría</strong><span>Histórico disponible · mes actual desde Movimientos · monto + % sobre ingreso regular</span></div><button type="button" class="text-btn" id="flowOriginalOrderV2">Orden original</button></div><div class="flow-matrix-scroll"><table class="flow-matrix-advanced"><thead><tr><th class="sticky-id" rowspan="2" data-sort="id">ID${arrow('id')}</th><th class="sticky-cat" rowspan="2" data-sort="category">Categoría${arrow('category')}</th>${months.map(m=>`<th colspan="2" data-sort-month="${m}">${esc(monthLabel(m))}${arrow('month',m)}</th>`).join('')}</tr><tr>${months.map(()=>'<th>Monto</th><th>% ingreso</th>').join('')}</tr></thead><tbody>${ordered.map(({cat,id})=>`<tr><td class="sticky-id">${id}</td><td class="sticky-cat">${esc(cat)}</td>${months.map(m=>{const amount=values.get(`${m}|${norm(cat)}`)||0,base=salary.map.get(m)||0,share=base?amount/base:0;return`<td><button type="button" class="matrix-amount-btn" data-detail data-category="${esc(cat)}" data-month="${m}">${esc(money(amount))}</button></td><td><span class="matrix-pct ${pctClass(share)}">${base?esc(pct(share)):'—'}</span></td>`;}).join('')}</tr>`).join('')}<tr class="matrix-total-row"><td class="sticky-id"></td><td class="sticky-cat">Total gastado por categorías</td>${months.map(m=>{const total=m===current?currentData.total:categories.reduce((s,c)=>s+(values.get(`${m}|${norm(c)}`)||0),0),base=salary.map.get(m)||0,share=base?total/base:0;return`<td>${esc(money(total))}</td><td><span class="matrix-pct ${pctClass(share)}">${base?esc(pct(share)):'—'}</span></td>`;}).join('')}</tr>${SUMMARY_ORDER.map(label=>`<tr class="matrix-summary-row"><td class="sticky-id"></td><td class="sticky-cat">${esc(label)}</td>${months.map(m=>{const amount=m===current?(currentData.summary.get(norm(label))||0):(historical.get(`${m}|${norm(label)}`)||0),base=salary.map.get(m)||0,share=base?amount/base:0;return`<td>${esc(money(amount))}</td><td><span class="matrix-pct ${pctClass(share)}">${base?esc(pct(share)):'—'}</span></td>`;}).join('')}</tr>`).join('')}</tbody></table></div><div class="salary-reference"><strong>Salario / ingreso regular usado para los porcentajes</strong><div class="salary-reference-grid">${months.map(m=>`<div><span>${esc(monthLabel(m))}</span><strong>${salary.map.has(m)?esc(money(salary.map.get(m))):'—'}</strong><small>${esc(salary.source.get(m)||'Sin base disponible')}</small></div>`).join('')}</div></div><div class="matrix-color-legend"><span>0–5% blanco</span><span>6–10% verde</span><span>11–15% amarillo</span><span>&gt;15% rojo</span></div>`;
    host.querySelector('#flowOriginalOrderV2')?.addEventListener('click',()=>{sortState={type:'id',dir:'asc',month:null};render(host,detail,data);});
  ...
  async function run(force=false){
    ...
    try{
      ...
      const base=findBasePanel(); let host=root.querySelector('#flowMatrixV2'),detail=root.querySelector('#flowMatrixDetailV2');
      if(!host){ if(!base)return; host=document.createElement('div');host.id='flowMatrixV2';host.className='panel table-panel';base.insertAdjacentElement('afterend',host); detail=document.createElement('div');detail.id='flowMatrixDetailV2';detail.className='panel table-panel';detail.hidden=true;host.insertAdjacentElement('afterend',detail); }
      if(base)base.style.display='none';
      root.querySelector('#flowMatrixAdvanced')?.remove(); root.querySelector('#flowMatrixMigrationDetail')?.remove();
      ...
      document.dispatchEvent(new CustomEvent('panel:flow-matrix-v2-rendered'));
    }catch(e){console.error('Matriz Flujo v2:',e);}finally{applying=false;}
  }

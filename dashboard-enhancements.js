(() => {
  ...
  function parseDate(value){
    if(typeof value==='number'&&Number.isFinite(value)&&value>20000&&value<80000){const utc=new Date(Math.round((value-25569)*86400000));return new Date(utc.getUTCFullYear(),utc.getUTCMonth(),utc.getUTCDate());}const s=String(value??'').trim();if(!s)return null;let m=s.match(/^(\d{4})-(\d{1,2})(?:-(\d{1,2}))?/);if(m)return new Date(+m[1],+m[2]-1,+(m[3]||1));m=s.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})/);if(m)return new Date(+m[3],+m[2]-1,+m[1]);const d=new Date(s);return Number.isNaN(d.getTime())?null:d;}
  ...
  const cardLimit=card=>(String(window.__PANEL_CARD_LIMIT_MODE__||'control')==='real'?cardRealLimit(card):cardControlLimit(card));
  ...
  function renderCardPanelSkeleton(){if(activeView()!=='tarjetas')return null;const existing=document.querySelector('[data-card-line-panel]');if(existing)return existing;const anchor=document.getElementById('cardsChart')?.closest('.panel');if(!anchor)return null;const panel=document.createElement('div');panel.className='panel card-line-panel';panel.dataset.cardLinePanel='true';panel.innerHTML=`<div class="panel-header card-line-header"><div class="panel-title"><strong>Evolución por tarjeta</strong><span>Consulta gastos o porcentaje del límite utilizado</span></div><div class="card-line-controls"><div class="chart-mode-switch" role="group" aria-label="Métrica del gráfico"><button type="button" class="chart-mode-btn${cardMetric==='spend'?' active':''}" data-card-line-mode="spend">Gastos</button><button type="button" class="chart-mode-btn${cardMetric==='limit'?' active':''}" data-card-line-mode="limit">Límite utilizado</button></div><div class="card-limit-reference-slot"></div></div></div><div class="card-line-status" data-card-line-status>Cargando histórico…</div><div class="chart-scroll card-line-scroll" hidden><div class="chart-inner card-line-inner" style="width:760px;min-width:100%;height:330px"><canvas id="cardTrendChart"></canvas></div></div>`;anchor.insertAdjacentElement('afterend',panel);panel.querySelectorAll('[data-card-line-mode]').forEach(btn=>btn.addEventListener('click',()=>{cardMetric=btn.dataset.cardLineMode||'spend';panel.querySelectorAll('[data-card-line-mode]').forEach(x=>x.classList.toggle('active',x===btn));schedule();}));return panel;}
  ...
  function drawFixedYAxis(chart){const scroller=chart?.canvas?.closest('.chart-scroll'),scale=chart?.scales?.y;if(!scroller||!scale)return;let axis=scroller.querySelector(':scope > .fixed-y-axis');if(!axis){axis=document.createElement('div');axis.className='fixed-y-axis';scroller.prepend(axis);}axis.style.height=`${chart.height}px`;if(chart?.canvas?.id==='cardTrendChart'){const axisWidth=Math.max(36,Math.ceil(Number(scale.width)||Number(scale.right)||0));axis.style.width=`${axisWidth}px`;axis.style.minWidth=`${axisWidth}px`;axis.style.marginRight=`-${axisWidth}px`;}else{axis.style.width='';axis.style.minWidth='';axis.style.marginRight='';}axis.innerHTML='';(scale.ticks||[]).forEach((tick,index)=>{const y=scale.getPixelForTick(index);if(!Number.isFinite(y))return;const item=document.createElement('span');item.className='fixed-y-tick';item.style.top=`${y}px`;const label=formatAxisLabel(scale,tick,index);item.textContent=label.length>18?`${label.slice(0,17)}…`:label;item.title=label;axis.appendChild(item);});}

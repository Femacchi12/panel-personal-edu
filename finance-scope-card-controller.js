(() => {
  ...
  function injectStyles() {
    ...
    const style = document.createElement('style'); style.id = 'financeScopeCardStyles'; style.textContent = `
      ...
      @media(max-width:900px){.card-expense-summary{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:560px){.finance-scope-filter{min-width:0}.card-expense-summary{grid-template-columns:1fr}}
  ...
  function renderCardExpensePanel() {
    ...
    let tableRows = visibleRows.slice().sort((a, b) => (rowDate(b)?.getTime() || 0) - (rowDate(a)?.getTime() || 0));
    if (query) { const q = norm(query); tableRows = tableRows.filter(row => norm(Object.values(row).join(' ')).includes(q)); }
    const shown = expanded ? tableRows : tableRows.slice(0, 20);
    ...
    host.innerHTML = `<div class="panel-header"><div class="panel-title"><strong>Gastos realizados con tarjeta de crédito</strong><span>${active ? `Tarjeta: ${esc(cardLabel(active))} · ` : ''}${esc(scopeState.tarjetas)} · ${visibleRows.length} movimientos · ${esc(money(visibleTotal, currency))}</span></div><div class="table-toolbar"><input id="cardExpenseSearch" class="search-input" placeholder="Buscar gasto…" value="${esc(query)}"></div></div>
      <div class="card-expense-summary"><div><span>Total crédito</span><strong>${esc(money(total, currency))}</strong></div><div><span>Personal</span><strong>${esc(money(personal, currency))}</strong></div><div class="fibrazo"><span>FIBRAZO</span><strong>${esc(money(fibrazo, currency))}</strong></div><div class="fibrazo"><span>Por recuperar de FIBRAZO</span><strong>${esc(money(fibrazo, currency))}</strong></div></div>
      <div class="table-scroll${expanded ? ' expanded' : ''}"><table class="card-expense-table"><thead><tr><th>Fecha</th><th>Tarjeta</th><th>Titular</th><th>Ámbito</th><th>Categoría</th><th>Subcategoría</th><th>Descripción</th><th>Modalidad</th><th>Cuotas</th><th>Monto ${esc(currency)}</th></tr></thead><tbody>${shown.map(row => {
      ...
      ${tableRows.length > 20 ? `<div class="table-footer"><span>${expanded ? `Mostrando ${tableRows.length}` : `Mostrando 20 de ${tableRows.length}`}</span><button type="button" class="table-more" id="cardExpenseMore">${expanded ? 'Ver menos' : 'Ver más'}</button></div>` : ''}`;
  ...
  document.addEventListener('panel:card-filter-changed', event => { if (activeView() === 'tarjetas') { window.__PANEL_ACTIVE_CARD_ID__ = String(event.detail?.cardId || ''); schedule(); } });
  ...
  document.addEventListener('panel:expense-scope-changed', event => { if (event.detail?.view === 'tarjetas' && activeView() === 'tarjetas') schedule(); });

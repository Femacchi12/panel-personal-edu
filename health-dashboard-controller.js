(() => {
  ...
  function overdueTreatments(rows,view='salud'){
    ...
    return filtered(rows,view).filter(r=>{
      ...
      const open=status.includes('activ')||status.includes('seguimiento')||status.includes('mantenimiento');
      return planned&&planned<t&&!real&&open;
    });
  }
  ...
  function healthSummaryMarkup(data,map){
    ...
    return `<section class="health-enhancement" aria-label="Control documental de salud">
      <div class="health-ops-strip">
        ...
        <div><strong>${overdue.length}</strong><span>tratamientos con fecha a revisar</span></div>
      </div>
  ...
  async function loadData(){
    ...
    const [patients,docs,appointments,treatments,studies]=await Promise.all([
      get(HEALTH_ID,RANGES.patients,false),get(HEALTH_ID,RANGES.docs,false),get(HEALTH_ID,RANGES.appointments,false),get(HEALTH_ID,RANGES.treatments,false),get(HEALTH_ID,RANGES.studies,false)
    ]);
    return {patients:parseRows(patients),docs:parseRows(docs),appointments:parseRows(appointments),treatments:parseRows(treatments),studies:parseRows(studies)};
  }

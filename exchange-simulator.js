(() => {
  ...
  let active=false,renderVersion=0;
  ...
  async function payload(force=false){
    ...
    if(typeof getData==='function')return getData(force);
  ...
  async function render(force=false){
    ...
    if(version!==renderVersion||!active||!p)return;
  ...
  document.addEventListener('click',event=>{
    ...
    if(nav&&nav.dataset.view!=='cambio'&&active){active=false;renderVersion++;restoreChrome();}
    if(active&&event.target.closest('#refreshBtn'))setTimeout(()=>render(false),350);
  });
})();

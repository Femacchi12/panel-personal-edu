(() => {
  ...
  function ratesFromMatrix(matrix){return{usdCop:num(matrix?.[3]?.[1]),usdArs:num(matrix?.[4]?.[1])};}
  ...
  function paint(root,rates,items){
    setChrome();
    const arsCop=rates.usdCop&&rates.usdArs?new Intl.NumberFormat('es-CO',{maximumFractionDigits:4}).format(rates.usdCop/rates.usdArs):'—';
    const copArs=rates.usdCop&&rates.usdArs?new Intl.NumberFormat('es-CO',{maximumFractionDigits:4}).format(rates.usdArs/rates.usdCop):'—';
    root.innerHTML=`<div class="section-head" data-fx-view><div><span class="eyebrow">FINANZAS</span><h2>Simulador de tipo de cambio</h2></div><p>Prueba escenarios COP / USD / ARS sin modificar los movimientos históricos.</p></div>
      <div class="panel fx-panel"><div class="panel-header"><div class="panel-title"><strong>Tasas para la simulación</strong><span>Se cargan desde Simulador_TC; si falta una tasa debes ingresarla para simular.</span></div><a class="btn btn-secondary" target="_blank" rel="noopener" href="https://docs.google.com/spreadsheets/d/${financeId}/edit#gid=1900000001">Abrir hoja</a></div>
        <div class="fx-rate-grid"><label>1 USD = COP<input id="fxUsdCop" type="number" step="0.01" value="${rates.usdCop||''}" placeholder="Sin dato"></label><label>1 USD = ARS<input id="fxUsdArs" type="number" step="0.01" value="${rates.usdArs||''}" placeholder="Sin dato"></label><div><span>1 ARS = COP</span><strong id="fxArsCop">${arsCop}</strong></div><div><span>1 COP = ARS</span><strong id="fxCopArs">${copArs}</strong></div></div>
      </div>
    ...
    const updateCross=()=>{const a=num(root.querySelector('#fxUsdCop')?.value),b=num(root.querySelector('#fxUsdArs')?.value);const arsCopEl=root.querySelector('#fxArsCop'),copArsEl=root.querySelector('#fxCopArs');if(arsCopEl)arsCopEl.textContent=a&&b?new Intl.NumberFormat('es-CO',{maximumFractionDigits:4}).format(a/b):'—';if(copArsEl)copArsEl.textContent=a&&b?new Intl.NumberFormat('es-CO',{maximumFractionDigits:4}).format(b/a):'—';};

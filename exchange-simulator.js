(() => {
  ...
  async function payload(force=false){if(!force&&cache&&Date.now()-cacheAt<55_000)return cache;const token=await window.__PANEL_GET_ID_TOKEN__?.(false);if(!token)throw new Error('Sesión no disponible');const r=await fetch(`${apiBaseUrl}/api/data`,{headers:{Authorization:`Bearer ${token}`},cache:'no-store'});if(!r.ok)throw new Error(`Backend ${r.status}`);cache=await r.json();cacheAt=Date.now();return cache;}
  ...
  function paint(root,rates,items){
    setChrome();
    root.innerHTML=`<div class="section-head"><div><span class="eyebrow">FINANZAS</span><h2>Simulador de tipo de cambio</h2></div><p>Prueba escenarios COP / USD / ARS sin modificar los movimientos históricos.</p></div>
    ...
    const updateCross=()=>{const a=num(root.querySelector('#fxUsdCop').value),b=num(root.querySelector('#fxUsdArs').value);root.querySelector('#fxArsCop').textContent=b?new Intl.NumberFormat('es-CO',{maximumFractionDigits:4}).format(a/b):'—';root.querySelector('#fxCopArs').textContent=a?new Intl.NumberFormat('es-CO',{maximumFractionDigits:4}).format(b/a):'—';};
    ...
    root.querySelector('#fxCalculate').addEventListener('click',()=>{const usdCop=num(root.querySelector('#fxUsdCop').value),usdArs=num(root.querySelector('#fxUsdArs').value),amount=num(root.querySelector('#fxAmount').value),from=root.querySelector('#fxFrom').value,to=root.querySelector('#fxTo').value;if(!usdCop||!usdArs||!amount){root.querySelector('#fxResult').textContent='Completa tasas y monto';return;}const result=convert(amount,from,to,{usdCop,usdArs});root.querySelector('#fxResult').textContent=`${fmt(amount,from)} = ${fmt(result,to)}`;const at=new Intl.DateTimeFormat('es-CO',{year:'numeric',month:'short',day:'2-digit',hour:'2-digit',minute:'2-digit'}).format(new Date());const next=saveHistory({at,from,to,amount,result,usdCop,usdArs});root.querySelector('#fxHistory').innerHTML=historyHtml(next);});
    root.querySelector('#fxClearHistory').addEventListener('click',()=>{localStorage.removeItem(STORAGE_KEY);root.querySelector('#fxHistory').innerHTML=historyHtml([]);});
  }

  async function render(force=false){if(!active)return;const root=document.getElementById('viewRoot');if(!root)return;setChrome();const p=await payload(force).catch(e=>{console.error('Simulador TC:',e);return null;});const matrix=p?.sources?.[`${financeId}|Simulador_TC!A:J`]||[];paint(root,ratesFromMatrix(matrix),history());}

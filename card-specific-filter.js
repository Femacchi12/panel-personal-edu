(() => {
  ...
  function renderOptions(root){
    const signature=`${activeCardId}|${cards.map(card=>`${cardId(card)}:${cardLabel(card)}`).join('|')}`;if(root.dataset.signature===signature)return;root.dataset.signature=signature;
    const selected=selectedCard(),summary=root.querySelector('.card-specific-summary');if(summary)summary.textContent=selected?cardLabel(selected):'Todas';root.classList.toggle('has-selection',Boolean(selected));
    ...
    box.querySelectorAll('.card-specific-option').forEach(button=>button.addEventListener('click',event=>{event.stopPropagation();const id=String(button.dataset.cardId||'');activeCardId=activeCardId===id?'':id;window.__PANEL_ACTIVE_CARD_ID__=activeCardId;root.dataset.signature='';renderOptions(root);root.classList.remove('open');root.querySelector('.card-specific-trigger')?.setAttribute('aria-expanded','false');document.dispatchEvent(new CustomEvent('panel:card-filter-changed',{detail:{cardId:activeCardId}}));refreshCardsView();}));
  }

  function wireSectionClear(bar){const button=bar.querySelector('#clearSectionFilters');if(!button||button.dataset.cardSpecificWired==='1')return;button.dataset.cardSpecificWired='1';button.addEventListener('click',()=>{activeCardId='';window.__PANEL_ACTIVE_CARD_ID__='';const root=bar.querySelector('[data-card-specific-filter]');if(root){root.dataset.signature='';renderOptions(root);}document.dispatchEvent(new CustomEvent('panel:card-filter-changed',{detail:{cardId:''}}));},true);}

  function ensureCardFilterUI(){
    if(activeView()!=='tarjetas')return;const bar=document.getElementById('sectionFilterBar');if(!bar||bar.hidden)return;const grid=bar.querySelector('.section-filter-grid');if(!grid)return;
    ...
    if(!root){
      ...
      root.querySelector('.card-specific-trigger')?.addEventListener('click',event=>{event.stopPropagation();root.classList.toggle('open');root.querySelector('.card-specific-trigger')?.setAttribute('aria-expanded',root.classList.contains('open')?'true':'false');if(root.classList.contains('open'))setTimeout(()=>root.querySelector('.card-specific-search')?.focus(),0);});
      ...
      root.querySelector('.card-specific-clear')?.addEventListener('click',event=>{event.stopPropagation();if(!activeCardId)return;activeCardId='';window.__PANEL_ACTIVE_CARD_ID__='';root.dataset.signature='';renderOptions(root);document.dispatchEvent(new CustomEvent('panel:card-filter-changed',{detail:{cardId:''}}));refreshCardsView();});
    }

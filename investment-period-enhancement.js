(() => {
  ...
  function applyLocalFilters(rows){
    ...
    return rows.filter(row=>Object.entries(rules).every(([key,field])=>{
      ...
      return !selected.length||selected.includes(String(row[field]||'').trim());
    }));
  }
  ...
  function aggregate(rows,keyFn,valueFn){
    const map=new Map();
    rows.forEach(r=>{const k=keyFn(r);map.set(k,(map.get(k)||0)+(Number(valueFn(r))||0));});
    return map;
  }
  ...
  function buildTimeline(rows,currency,bounds){
    const items=rows.map(row=>{
      const d=parseDate(row.Fecha);
      return d?{date:d.getTime(),platform:String(row['Plataforma / Bróker']||'Sin plataforma').trim(),value:parseNumber(row[`Valor ${currency}`])}:null;
    }).filter(Boolean);
    ...
    const valueAt=(platform,date)=>{
      let best=-Infinity,value=null;
      maps.get(platform)?.forEach((v,t)=>{if(t<=date&&t>best){best=t;value=v;}});
      return value;
    };

    const datasets=[];
    if(platforms.length>1){
      datasets.push({label:'Portafolio consolidado',data:dates.map(d=>platforms.reduce((s,p)=>s+(valueAt(p,d)||0),0)),borderColor:COLORS[0],backgroundColor:COLORS[0],borderWidth:3,tension:.22,spanGaps:true});
    }
  ...
  function chartOptions(horizontal=false){
    ...
    return {
      responsive:true,maintainAspectRatio:false,indexAxis:horizontal?'y':'x',interaction:{mode:'nearest',intersect:false},
      ...
      scales:{x:{ticks:{color:'#718098',maxRotation:0},grid:{color:'#121c29'}},y:{beginAtZero:true,ticks:{color:'#718098'},grid:{color:'#121c29'}}}
    };
  }
  ...
  async function render(force=false){
    ...
    const line=buildTimeline(raw,currency,bounds);
    ...
    const pc=document.getElementById('investmentPeriodPlatformChart');
    if(pc) charts.push(new Chart(pc,{type:'bar',data:{labels:[...byPlatform.keys()],datasets:[{label:`Valor ${currency}`,data:[...byPlatform.values()]}]},options:chartOptions(false)}));
    const cats=[...byCategory.entries()].sort((a,b)=>b[1]-a[1]).slice(0,14);
    const cc=document.getElementById('investmentPeriodCategoryChart');
    if(cc) charts.push(new Chart(cc,{type:'bar',data:{labels:cats.map(x=>x[0]),datasets:[{label:`Valor ${currency}`,data:cats.map(x=>x[1])}]},options:chartOptions(true)}));
    const lc=document.getElementById('investmentPeriodTimelineChart');
    if(lc&&line.labels.length) charts.push(new Chart(lc,{type:'line',data:{labels:line.labels,datasets:line.datasets},options:chartOptions(false)}));
  }
  ...
  document.addEventListener('click',event=>{
    if(event.target.closest('.nav-item')) setTimeout(()=>{forcePeriodFilters();schedule(false,260);},100);
  ...
  if(filterBar){
    new MutationObserver(()=>{if(activeView()==='inversiones')forcePeriodFilters();})
      .observe(filterBar,{attributes:true,subtree:true,attributeFilter:['hidden']});
  }

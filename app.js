(() => {
  ...
  function serviceReferenceCards(rows){
    ...
    return `<div class="service-reference-grid">${rows.filter(r=>!pick(r,['Estado'])||norm(pick(r,['Estado'])).includes('activ')).map(r=>{
      ...
      return `<article class="service-reference-card"><div class="service-reference-head"><div><span>${esc(pick(r,['Tipo referencia'])||'Referencia')}</span><strong>${esc(pick(r,['Nombre'])||'Ubicación')}</strong></div><span class="badge">Actual</span></div><div class="service-reference-location"><strong>${esc([pick(r,['Ciudad']),pick(r,['Barrio'])].filter(Boolean).join(' · '))}</strong><span>${esc(address||'—')}</span></div><div class="service-reference-facts"><div><span>Código postal</span><strong>${esc(pick(r,['Código postal'])||'—')}</strong></div><div><span>Coordenadas</span><strong>${esc(coords||'—')}</strong></div></div><div class="service-reference-note"><span>Referencia</span><p>${esc(pick(r,['Referencia'])||'—')}</p></div><div class="service-reference-actions">${maps?`<a class="button secondary" href="${esc(maps)}" target="_blank" rel="noopener noreferrer">Abrir en Maps</a>`:''}${photo?`<a class="button secondary" href="${esc(photo)}" target="_blank" rel="noopener noreferrer">Ver foto</a>`:''}</div></article>`;
    }).join('')}</div>`;
  }

  function renderServicios() {const all=state.data.servicios||[];const rows=filterByPeriod(all);const display=rows.length?rows:all;const paid=display.filter(r=>norm(pick(r,['Estado mes','Estado'])).includes('pagad'));const pending=display.filter(r=>norm(pick(r,['Estado mes','Estado'])).includes('pend'));const references=state.data.referenciasPersonales||[];return `${sectionHead('FINANZAS','Servicios y referencias','Pagos mensuales, vencimientos y datos de referencia')}<div class="kpi-grid">${kpi('Servicios',String(display.length),'Servicios registrados')}${kpi('Pagados',String(paid.length),'Mes visible','green')}${kpi('Pendientes',String(pending.length),'Mes visible','gold')}${kpi('Pagado mes',money(sum(display,r=>num(pick(r,['Pagado mes COP','Monto COP','Valor'])))),'Total registrado')}</div>${serviceReferenceCards(references)}${chartPanel('Pago por servicio','Monto pagado en el período','servicesChart',Math.max(760,display.length*100))}${tablePanel('Servicios',display,['Servicio','Tipo de servicio','Tipo de referencia','Número de referencia','Dirección / ubicación','Día de pago','Mes control','Pagado mes COP','Estado mes','Próximo vencimiento','Observaciones'])}${tablePanel('Cuentas y plataformas',state.data.cuentas||[])}`;}

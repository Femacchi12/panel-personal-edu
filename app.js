(() => {
  ...
  const SOURCES = [
    ...
    {key:'docsFinancieros', book:'documents', range:'Documentos_Financieros!A:L', parser:'smart'},
  ...
  function renderGastos() {const rows=filteredMovements().filter(isExpense);const total=sum(rows,movementAmount);const avg=rows.length?total/rows.length:0;const categories=unique(rows.map(r=>pick(r,['Categoría','Categoria'])).filter(Boolean));return `${sectionHead('FINANZAS','Detalle de gastos','Histórico de consumos, comparación y base detallada')}<div class="kpi-grid">${kpi('Gasto seleccionado',money(total),`${rows.length} movimientos`)}${kpi('Ticket promedio',money(avg),'Promedio por movimiento')}${kpi('Categorías',String(categories.length),'Con gasto en el filtro')}${kpi('Financiado',money(sum(rows.filter(isFinanced),movementAmount)),'Tarjetas / cuotas','gold')}</div>${chartPanel('Evolución de gastos','Series por categoría / selección','spendChart',Math.max(760,periodCount(rows)*100))}${tablePanel('Movimientos',rows,['Fecha real','Tipo','Categoría','Subcategoría','Descripción / Comercio','Monto original','Moneda original','Cuenta / Tarjeta','Titular','Cuotas','N° cuota','Estado','Monto COP','Monto ARS','Monto USD'])}`;}
  ...
  function renderTarjetas() {const rows=state.data.tarjetas||[];const used=sum(rows,r=>num(pick(r,['Cupo usado','Utilizado','Saldo usado'])));const total=sum(rows,r=>num(pick(r,['Cupo total actual','Cupo total','Límite','Limite','Cupo'])));return `${sectionHead('FINANZAS','Tarjetas de crédito','Uso, cupo, facturación, pagos y nivel de utilización')}<div class="kpi-grid">${kpi('Cupo total',money(total),`${rows.length} tarjetas`)}${kpi('Cupo usado',money(used),total?`${formatNumber(used/total*100,1)}% consolidado`:'—','gold')}${kpi('Disponible',money(Math.max(0,total-used)),'Cupo consolidado','green')}${kpi('Pago próximo',money(sum(rows,r=>num(pick(r,['Pago total próximo','Pago mínimo próximo'])))),'Suma registrada')}</div><div class="credit-grid">${rows.length?rows.map(creditCard).join(''):empty('No hay tarjetas registradas')}</div>${chartPanel('Uso por tarjeta','Cupo usado vs disponible','cardsChart',760)}${tablePanel('Detalle de tarjetas',rows,['Emisor','Producto','Titular','Moneda','Cupo total actual','Cupo usado','Cupo disponible','% utilización','Día corte','Día vencimiento','Pago mínimo próximo','Pago total próximo','Fecha actualización','Observaciones','Límite personal de gasto','% uso límite personal'])}`;}
  function renderDeudas() {const all=state.data.cuotas||[];const rows=all.filter(r=>!/pagad[ao]/i.test(pick(r,['Estado'])));const pending=sum(rows,r=>num(pick(r,['Saldo pendiente','Saldo','Valor pendiente'])));const installment=sum(rows,r=>num(pick(r,['Valor cuota','Cuota'])));const interest=sum(rows,r=>num(pick(r,['Intereses'])));return `${sectionHead('FINANZAS','Deudas y cuotas','Compras financiadas, cuotas e intereses pendientes')}<div class="kpi-grid">${kpi('Saldo pendiente',money(pending),`${rows.length} cuotas activas`,'gold')}${kpi('Valor cuotas',money(installment),'Suma de cuotas activas')}${kpi('Intereses',money(interest),'Intereses registrados','red')}${kpi('Compras',String(unique(rows.map(r=>pick(r,['ID compra','Descripción'])).filter(Boolean)).length),'Compras con saldo')}</div>${chartPanel('Saldo pendiente por compra','Principales deudas activas','debtChart',Math.max(760,rows.length*75))}${tablePanel('Cuotas pendientes',rows,['Fecha compra','Comercio','Descripción','Tarjeta','Titular','Total compra','N° cuotas','Cuota actual','Valor cuota','Saldo pendiente','Intereses','Fecha última cuota','Estado'])}`;}
  function renderInversiones() {const posAll=state.data.posiciones||[];const pos=latestSnapshot(posAll,'Fecha');const value=sum(pos,positionAmount);const byPlatform=aggregate(pos,r=>pick(r,['Plataforma / Bróker','Plataforma','Bróker'])||'Sin plataforma',positionAmount);const byCategory=aggregate(pos,r=>pick(r,['Categoría','Categoria','Clase de activo'])||'Sin categoría',positionAmount);return `${sectionHead('FINANZAS','Inversiones','Posiciones, plataformas y composición del portafolio')}<div class="kpi-grid">${kpi('Portafolio',money(value),`${pos.length} posiciones`,'green')}${kpi('Plataformas',String(byPlatform.size),'Con posiciones')}${kpi('Categorías',String(byCategory.size),'Composición')}${kpi('Fecha corte',latestDateLabel(pos,'Fecha')||'—','Último snapshot disponible')}</div><div class="panel-grid equal">${chartPanel('Por plataforma','Valor de mercado','investmentPlatformChart',760)}${chartPanel('Por categoría','Distribución de posiciones','investmentCategoryChart',760)}</div>${tablePanel('Posiciones',pos,['Fecha','Plataforma / Bróker','Símbolo','Instrumento','Clase de activo','Categoría','Subcategoría','Cantidad','Precio','Valor USD','Valor COP','Valor ARS','Fuente'])}${tablePanel('Resumen de inversiones',state.data.inversiones||[])}`;}
  ...
  function renderDocumentos() {const rows=state.data.documentos||[];const dated=filterByPeriod(rows);const display=dated.length?dated:rows;return `${sectionHead('VIDA','Documentos','Documentos financieros, personales, laborales, tributarios, previsionales y de salud')}<div class="kpi-grid">${kpi('Documentos',String(display.length),'Registros consolidados')}${kpi('Áreas',String(unique(display.map(r=>pick(r,['Área','Tipo','Categoría','Producto'])).filter(Boolean)).length),'Clasificaciones')}${kpi('PDF',String(display.filter(r=>/pdf/i.test(JSON.stringify(r))).length),'Archivos / referencias PDF')}${kpi('Salud',String(display.filter(r=>/PAC-|salud|medic|dermat|odonto|otorr/i.test(JSON.stringify(r))).length),'Relacionados con salud','blue')}</div>${chartPanel('Documentos por área / tipo','Distribución del archivo','documentsChart',760)}${tablePanel('Índice de documentos',display)}`;}
  ...
  function chartPanel(title,subtitle,id,width=760) {return `<div class="panel"><div class="panel-header"><div class="panel-title"><strong>${esc(title)}</strong><span>${esc(subtitle)}</span></div></div>${chartShell(id,width)}</div>`;}
  function drawViewCharts() {try{const map={general:drawGeneralCharts,gastos:drawSpendChart,flujo:drawFlowChart,tarjetas:drawCardsChart,deudas:drawDebtChart,inversiones:drawInvestmentCharts,pension:drawPensionChart,ingresos:drawIncomeChart,servicios:drawServicesChart,salud:drawHealthCharts,citas:drawAppointmentsChart,tratamientos:drawTreatmentsChart,documentos:drawDocumentsChart,viajes:drawTravelChart};map[state.view]?.();}catch(error){console.error('Error dibujando gráficos:',error);}}
  ...
  function drawDebtChart(){const rows=(state.data.cuotas||[]).filter(r=>!/pagad[ao]/i.test(pick(r,['Estado'])));const agg=aggregate(rows,r=>pick(r,['Descripción','Comercio'])||'Compra',r=>num(pick(r,['Saldo pendiente','Saldo'])));const top=[...agg.entries()].sort((a,b)=>b[1]-a[1]).slice(0,12);makeBar('debtChart',top.map(x=>x[0]),[{label:'Saldo pendiente',data:top.map(x=>x[1]),backgroundColor:COLORS[1]}],true);}
  function drawInvestmentCharts(){const pos=latestSnapshot(state.data.posiciones||[],'Fecha');const byPlatform=[...aggregate(pos,r=>pick(r,['Plataforma / Bróker','Plataforma'])||'Sin plataforma',positionAmount).entries()].sort((a,b)=>b[1]-a[1]);makeBar('investmentPlatformChart',byPlatform.map(x=>x[0]),[{label:`Valor ${state.currency}`,data:byPlatform.map(x=>x[1]),backgroundColor:COLORS[0]}],true);const byCat=[...aggregate(pos,r=>pick(r,['Categoría','Categoria','Clase de activo'])||'Sin categoría',positionAmount).entries()].sort((a,b)=>b[1]-a[1]).slice(0,12);makeBar('investmentCategoryChart',byCat.map(x=>x[0]),[{label:`Valor ${state.currency}`,data:byCat.map(x=>x[1]),backgroundColor:COLORS[2]}],true);}
  ...
  function latestSnapshot(rows,dateKey='Fecha'){if(!rows?.length)return[];const dated=rows.map(r=>({r,d:parseDate(r[dateKey]||pick(r,['Fecha','Fecha corte']))})).filter(x=>x.d);if(!dated.length)return rows;const max=Math.max(...dated.map(x=>x.d.getTime()));return dated.filter(x=>x.d.getTime()===max).map(x=>x.r);}
  ...
  function positionAmount(row){if(state.currency==='USD')return num(pick(row,['Valor USD','USD']));if(state.currency==='ARS')return num(pick(row,['Valor ARS','ARS']));return num(pick(row,['Valor COP','COP']));}

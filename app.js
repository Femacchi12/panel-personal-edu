(() => {
  ...
  function drawDebtChart(){const purchases=canonicalDebtPurchases(state.data.cuotas||[]);const top=purchases.map(p=>[p.label,p.outstanding]).sort((a,b)=>b[1]-a[1]).slice(0,12);makeBar('debtChart',top.map(x=>x[0]),[{label:'Principal pendiente',data:top.map(x=>x[1]),backgroundColor:COLORS[1]}],true);}

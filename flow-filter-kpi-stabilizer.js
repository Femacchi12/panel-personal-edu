(() => {
  ...
  function incomeReference(concepts){
    ...
    const copRegular=copActual||median(sameYear.map(r=>r['Sueldo COP']))||median(concepts.map(r=>r['Sueldo COP']));
    const usdRegular=usdActual||median(sameYear.map(r=>r['Sueldo USD (equiv. COP)']))||median(concepts.map(r=>r['Sueldo USD (equiv. COP)']));
  ...
  function primaryCards(){
    ...
    const grid=[...root.querySelectorAll('.kpi-grid')].find(g=>{
      const labels=[...g.querySelectorAll('.kpi-label')].map(x=>x.textContent.trim());
      return labels.some(x=>x==='Ingresos'||x==='Ingresos promedio')&&labels.includes('Egresos')&&labels.includes('Ahorro');
    });
    if(!grid)return null;
    const cards=[...grid.querySelectorAll('.kpi-card')];
    return {
      income:cards.find(c=>['Ingresos','Ingresos promedio'].includes(c.querySelector('.kpi-label')?.textContent.trim())),
      expense:cards.find(c=>c.querySelector('.kpi-label')?.textContent.trim()==='Egresos'),
      savings:cards.find(c=>c.querySelector('.kpi-label')?.textContent.trim()==='Ahorro'),
      rate:cards.find(c=>norm(c.querySelector('.kpi-label')?.textContent).includes('tasa de ahorro'))
    };
  }
  ...
  async function apply(force=false){
    ...
    try{
      ...
      const expenses=filtered.reduce((sum,r)=>sum+parseNumber(r[currency==='COP'?'Monto COP':currency==='USD'?'Monto USD':'Monto ARS']),0);
      const savings=income-expenses;
      const rate=income?savings/income:0;
      const cards=primaryCards();
      if(cards){
        setCard(cards.income,'Ingresos promedio',formatMoney(income,currency),ref.meta);
        setCard(cards.expense,'Egresos',formatMoney(expenses,currency),`${filtered.length} movimientos registrados según filtros`);

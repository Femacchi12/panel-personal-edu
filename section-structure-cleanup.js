(() => {
  ...
  function injectInvestmentGuard(){
    ...
    const style=document.createElement('style');style.id='investmentImmediateLegacyGuard';style.textContent=`
      ...
      body:has(.nav-item.active[data-view="inversiones"]) #viewRoot > #investmentPeriodCorrected{display:grid!important}
    `;document.head.appendChild(style);
  }

  function apply(){if(applying)return;applying=true;try{const root=document.getElementById('viewRoot');if(!root)return;cleanCards(root);cleanPension(root);cleanInvestments(root);}finally{applying=false;}}
